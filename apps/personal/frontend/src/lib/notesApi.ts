...
const BASE = "http://localhost:8000/api/notes";
...
export const listFolders = async (): Promise<NoteFolder[]> => {
  const data = await req<any>("/folders");
...
export const listNotes = async (folderId?: number): Promise<NoteListItem[]> => {
  const data = await req<any>(folderId != null ? `?folder_id=${folderId}` : "");
...
export const getBacklinks = async (id: number): Promise<NoteBacklink[]> => {
  const data = await req<any>(`/${id}/backlinks`);
  return Array.isArray(data) ? data : (data.backlinks ?? []);
};

// ─── Search & Graph ──────────────────────────────────────────────────────────

export const searchNotes = async (q: string, limit = 30): Promise<NoteSearchResult[]> => {
  const data = await req<any>(`/search?q=${encodeURIComponent(q)}&limit=${limit}`);
