...
import { HabitMain } from "@/components/habit/HabitMain";
...
import {
  ...
  Wallet, Package, NotebookPen, AlertTriangle, RefreshCw, Flame,
} from "lucide-react";
...
type Panel = "chat" | "workspace" | "analytics" | "finance" | "inventory" | "habit";
...
function MainLayout() {
  ...
  const handleConvCreated = useCallback((convId: string, _title: string) => {
  ...
  const navItems: { id: Panel; icon: React.ReactNode; label: string }[] = [
    ...
    { id: "habit",     icon: <Flame size={14} />,         label: t("habit.title")      },
    { id: "finance",   icon: <Wallet size={14} />,        label: t("persona.finance")  },
    { id: "inventory", icon: <Package size={14} />,       label: t("persona.inventory") },
      ...
      <header className="h-12 shrink-0 glass-surface border-b th-border flex items-center px-4 gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0 mr-1">
          ...
          <div className="flex flex-col leading-none">
            ...
            <span className="text-[8px] font-bold text-brand-400 uppercase tracking-[0.2em]">Home</span>
          </div>
        </div>
