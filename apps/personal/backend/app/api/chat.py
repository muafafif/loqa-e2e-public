...
async def generate_stream(request: ChatRequest):
    ...
    # Inject system prompt for unified mode (finance + habit)
    if request.chat_mode == "unified":
        unified_system = {
            "role": "system",
            "content": (
                f"{persona_prompt}\n\n"
                "You are LOQA Home, an all-in-one personal assistant with access to live data tools "
                "spanning finance and habits.\n\n"
                "AVAILABLE TOOL GROUPS:\n"
                "- Finance: get_finance_summary, list_transactions, get_monthly_report, "
                "get_pl_report, list_accounts, list_categories\n"
                "- Habit: get_today_habit_status, list_habits, get_habit_stats\n\n"
                "## CRITICAL RULES\n"
                ...
                "- NEVER explain what you are about to do. Just call the tool, then respond.\n\n"
                "## PHASE 1 — DATA RETRIEVAL\n"
                "For ANY question about personal data, call the most relevant tool immediately:\n"
                "- Finance totals/balance/transactions → get_finance_summary, list_transactions\n"
                "- Monthly finance trend → get_monthly_report\n"
                "- Today's habits / check-in status → get_today_habit_status\n"
                "- Habit list → list_habits\n"
                "- Habit streak / progress → get_habit_stats\n"
                "- For general or advice questions with no clear domain, call get_finance_summary first.\n\n"
                "## PHASE 2 — RESPONSE\n"
                "After tools return data, determine the response type:\n\n"
                ...
                "TYPE B — Advice only (e.g. 'beri saran', 'apa yang harus', 'bagaimana cara'):\n"
    ...
    try:
        async for item in llm_service.stream_chat(tool_enhanced_messages, app_settings.chat):
            ...
            # ── Stream filter ─────────────────────────────────────────────────
            # Pass 1: detect/capture <think>…</think> and emit thinking events.
            chunk = item
            pass1_out = ""
            for ch in chunk:
                if in_think:
            ...
            # Pass 2: strip all plain-text tool call token formats.
...
def _build_habit_context() -> str:
    """Build a plain-text habit summary to inject as system context."""
    try:
        ...
        if not habits:
            return "## Habits\nNo active habits."
        lines = [f"## Habits — Today ({today})"]
        done = [h for h in habits if h.get("checked_today")]
        pending = [h for h in habits if not h.get("checked_today")]
        lines.append(f"- Done: {len(done)}/{len(habits)}")
        for h in done:
        ...
        return "## Habits data not available"


def _build_unified_context(pocket_id: Optional[int]) -> str:
    """Build a combined finance + habit context snapshot for models without tool calling."""
    parts = [_build_finance_context(pocket_id), _build_habit_context()]
    return "\n\n".join(parts)
