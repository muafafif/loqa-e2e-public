...
import { Hash, MessageSquare, Database, Zap, RefreshCw, BookOpen, Activity, Clock, Brain, Download } from "lucide-react";
...
export function AnalyticsPanel({ sessionId }: Props) {
  ...
  const handleExport = async (_includeInsight: boolean) => {
    setExporting(true);
    try {
      await exportAnalyticsPdf({
        appName: "LOQA Work", period,
        periodData, sessionData, kbData,
      });
    } catch (err) {
      alert("PDF error: " + (err instanceof Error ? err.message : String(err)));
    } finally { setExporting(false); setExportDialogOpen(false); }
  };
    ...
    <div className="flex flex-col flex-1 overflow-y-auto scrollbar-thin min-w-0" style={{ background: "rgb(var(--bg-base))" }}>
      ...
      <div className="px-5 py-3 border-b flex items-center justify-between shrink-0 glass-surface" style={{ borderColor: "rgb(var(--border))" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setExportDialogOpen(true)} disabled={exporting || loading}
            ...
            <Download size={12} />
            {exporting ? t("common.exportingPdf") : t("common.exportPdf")}
          </button>
          <button
            onClick={load}
            disabled={loading}
