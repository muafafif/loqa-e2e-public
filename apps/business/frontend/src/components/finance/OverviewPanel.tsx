...
import { exportFinancePdf, generateAiInsight, buildFinancePrompt, buildPLPrompt } from "@/lib/exportPdf";
...
import {
  ...
  Download, ChevronDown, Lock, Info, BarChart2,
} from "lucide-react";
...
type KpiRange     = "30d" | "90d" | "180d" | "1y" | "custom";
type ReportRange  = "30d" | "90d" | "180d" | "1y";
type PLRange      = "3m" | "6m" | "12m";
type CategoryMetric = "income" | "expense" | "net";
...
function KpiSection({ pocketId, pockets, allAccounts, lockStatus, filterPocketId, setFilterPocketId, filterAccountId, setFilterAccountId }: SectionProps) {
  ...
  const [catMetric, setCatMetric]       = useState<CategoryMetric>("income");
  ...
  function toKpiDateRange(r: KpiRange): { from?: string; to?: string } {
    if (r === "custom") return { from: customFrom || undefined, to: customTo || undefined };
    const now = new Date();
  ...
  const load = useCallback(async () => {
    if (kpiRange === "custom" && (!customFrom || !customTo)) return;
    setLoading(true);
    try {
      const { from, to } = toKpiDateRange(kpiRange);
      setSummary(await getFinanceSummary(from, to, effectivePocketId, effectiveAccountId));
    } catch { /* ignore */ } finally { setLoading(false); }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kpiRange, customFrom, customTo, effectivePocketId, effectiveAccountId]);

  useEffect(() => { load(); }, [load]);

  const kpiRanges: { id: KpiRange; label: string }[] = [
    { id: "30d",    label: t("finance.summary.30d")    },
    { id: "90d",    label: t("finance.summary.90d")    },
    { id: "180d",   label: t("finance.summary.180d")   },
    { id: "1y",     label: t("finance.summary.1y")     },
    { id: "custom", label: t("finance.summary.custom") },
  ];

  const catMetrics: { id: CategoryMetric; label: string; color: string }[] = [
    { id: "income",  label: t("finance.summary.income"),  color: "#10b981" },
    { id: "expense", label: t("finance.summary.expense"), color: "#ef4444" },
    { id: "net",     label: t("finance.summary.net"),     color: "#a78bfa" },
  ];
  ...
  // By Category — filter by chosen metric
  const catLabel = (c: { name?: string | null }) =>
    (c.name ?? t("finance.tx.noCategory")).length > 18
      ? (c.name ?? "").slice(0,16)+"…"
      : (c.name ?? t("finance.tx.noCategory"));

  const barData = summary
    ? (summary.by_category ?? [])
        .filter(c => c.type !== "transfer")
        .filter(c => catMetric === "income" ? c.type === "income" : catMetric === "expense" ? c.type === "expense" : true)
        .slice(0, 10)
        .map(c => {
          const net = c.type === "income" ? c.total : -c.total;
          const value = catMetric === "net" ? net : c.total;
          const fill  = catMetric === "net" ? (net >= 0 ? "#a78bfa" : "#f97316") : c.color ?? "#6b7280";
          return { name: catLabel(c), value, fill, pct: summary.total_income > 0 ? Math.abs(value) / summary.total_income * 100 : 0 };
        })
        .sort((a, b) => Math.abs(b.value) - Math.abs(a.value))
    : [];

  const selectedCatMetric = catMetrics.find(m => m.id === catMetric)!;
  ...
  return (
    <div className="p-5 max-w-3xl mx-auto flex flex-col gap-5">
      ...
      <div className="flex items-center gap-2">
        <div className="tab-glass flex">
          {kpiRanges.map(({ id, label }) => (
            <button key={id} onClick={() => setKpiRange(id)}
              className={clsx("tab-glass-item btn-brand", kpiRange === id && "active")}>
              {label}
            </button>
          ))}
        </div>
      ...
      {kpiRange === "custom" && (
        <div className="flex items-center gap-2 flex-wrap -mt-2">
          <span className="text-xs th-text-muted">{t("finance.summary.from")}</span>
          <input type="date" value={customFrom} onChange={e => setCustomFrom(e.target.value)}
            className="px-2.5 py-1.5 rounded-lg text-xs th-bg-elevated th-text border th-border focus:outline-none focus:border-brand-500" />
          <span className="text-xs th-text-muted">{t("finance.summary.to")}</span>
      ...
      {loading ? <Spinner /> : summary ? (
        <>
                ...
                <div className="flex flex-col gap-3 flex-1">
                  ...
                  {summary.total_income > 0 && summary.total_expense > 0 && (
                    <div className="pt-2 border-t th-border text-[11px] th-text-muted flex justify-between">
                      <span>{t("finance.summary.expenseRatio")}</span>
          ...
          {/* By Category bar chart — with metric filter */}
          {(summary.by_category ?? []).filter(c => c.type !== "transfer").length > 0 && (
            <div className="glass-card p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[11px] font-bold uppercase tracking-[0.1em]" style={{ color: "rgb(var(--tx-muted))" }}>
                      {t("finance.summary.byCategory")}
                    </span>
                    <span className="ml-2 text-[11px] font-semibold" style={{ color: selectedCatMetric.color }}>
                      — {selectedCatMetric.label}
                      {catMetric === "net" && summary.total_income > 0 && (
                        <span className="ml-1 text-[10px] th-text-muted">
                          ({t("finance.summary.netPctOfIncome")})
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1">
                  {catMetrics.map(m => (
                    <button key={m.id} onClick={() => setCatMetric(m.id)}
                      className="px-2.5 py-1 rounded-full text-[11px] font-semibold transition-all"
                      style={catMetric === m.id
                        ? { background: m.color + "28", color: m.color, border: `1px solid ${m.color}50` }
                        : { color: "rgb(var(--tx-muted))", border: "1px solid rgb(var(--border))" }}>
                      {m.label}
                    </button>
                  ))}
                </div>
              ...
              {barData.length > 0 ? (
                <ResponsiveContainer width="100%" height={Math.max(barData.length * 38 + 16, 160)}>
                  <BarChart data={barData} layout="vertical" margin={{left:8,right:56,top:0,bottom:0}}>
                    ...
                    <Tooltip {...TOOLTIP_PROPS}
                      formatter={(v: unknown) => {
                        const n = Number(v);
                        if (catMetric === "net" && summary.total_income > 0) {
                          return [`${fmt(n)} (${(Math.abs(n)/summary.total_income*100).toFixed(1)}%)`, selectedCatMetric.label];
                        }
                        return [fmt(n), selectedCatMetric.label];
                      }}
                    />
                    <Bar dataKey="value" radius={[0,8,8,0]} maxBarSize={26} label={{ position: "right", formatter: (v: unknown) => fmtShort(Number(v)), style: { fill: "rgb(var(--tx-muted))", fontSize: 10 } }}>
                      {barData.map((d,i)=><Cell key={i} fill={d.fill} fillOpacity={0.92} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <p className="text-xs th-text-muted text-center py-6">{t("finance.report.noData")}</p>
              )}

              {catMetric === "net" && (
                <p className="text-[11px] th-text-muted">
                  {t("finance.summary.netCatNote")}
                </p>
              )}
            </div>
          )}
  ...
  const handleExport = async (includeInsight: boolean) => {
    ...
    try {
      ...
      await exportFinancePdf({
        appName:"LOQA Work", rangeLabel:rangeLabels[range]+(ctx?` · ${ctx}`:""),
...
function PLSection({ pocketId, pockets, allAccounts, lockStatus, filterPocketId, setFilterPocketId, filterAccountId, setFilterAccountId }: SectionProps) {
  ...
  const handleExport = async (includeInsight: boolean) => {
    ...
    try {
      ...
      await exportPLPdf({ appName:"LOQA Work", rangeLabel:rangeLabel+(ctx?` · ${ctx}`:""), rows, totals:{...totals,gross_margin:totalGrossMargin,net_margin:totalNetMargin}, aiInsight });
    } catch (err) { alert("PDF error: "+(err instanceof Error?err.message:String(err))); }
    finally { setExporting(false); setGeneratingInsight(false); setExportDialogOpen(false); }
  };
