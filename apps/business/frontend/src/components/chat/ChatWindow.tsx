...
import { WifiOff, UserCog, X, Check, Sparkles, Zap, Shield, Wallet, Package, TrendingUp, AlertTriangle, RefreshCw } from "lucide-react";
...
function CubeIcon({ mode }: { mode: "chat" | "doc" }) {
  ...
  // Particles: 6 dots per mode, positions & timings fixed
  const particles = [
  ...
  const particleColor = "rgb(var(--accent-300) / 0.85)";
  ...
  return (
    <div style={{ position: "relative", width: 100, height: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
      {/* Volumetric glow */}
      <div style={{
        position: "absolute", inset: -40, borderRadius: "50%", pointerEvents: "none", zIndex: 0,
        filter: "blur(28px)", animation: "cube-glow-breathe 6s ease-in-out infinite",
        background: `radial-gradient(circle at 50% 58%, ${glowColor} 0%, transparent 68%)`,
        animationDelay: isDoc ? "-1.8s" : "0s",
      }} />
      <div style={{
        position: "absolute", inset: 4, borderRadius: "50%", pointerEvents: "none", zIndex: 0,
        filter: "blur(12px)", animation: "cube-glow-breathe 6s ease-in-out infinite",
        background: `radial-gradient(circle at 50% 52%, ${glowInner} 0%, transparent 60%)`,
        animationDelay: isDoc ? "-2.3s" : "-0.5s",
      }} />
      ...
      <div style={{ position: "absolute", inset: -44, pointerEvents: "none", zIndex: 0 }}>
        {particles.map((p, i) => (
          <div key={i} style={{
            ...
            animation: `cube-particle linear ${p.dur} ${p.delay} infinite`,
          }} />
        ))}
      </div>
      ...
      <div style={{ width: 100, height: 100, perspective: 350, position: "relative", zIndex: 1 }}>
        <div style={{
          ...
          animation: `${animName} 8s cubic-bezier(.37,0,.63,1) infinite`,
          animationDelay: isDoc ? "-2s" : "0s",
        }}>
          {/* Front */}
          <div style={{
            position: "absolute", width: 60, height: 60,
            background: face.front, transform: "translateZ(30px)",
            boxShadow: "inset 0 1.5px 0 rgba(255,255,255,.35), inset 0 -1px 0 rgba(0,0,0,.1)",
            display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden",
          }}>
            {/* Sheen */}
            <div style={{
              position: "absolute", inset: 0, pointerEvents: "none",
              background: "radial-gradient(ellipse 70% 45% at 38% 28%, rgba(255,255,255,.38) 0%, transparent 100%), linear-gradient(135deg, rgba(255,255,255,.15) 0%, transparent 55%)",
            }} />
            {/* Reflect */}
            ...
            {isDoc ? (
              ...
              <svg style={{ position: "relative", zIndex: 1, flexShrink: 0 }} width="30" height="30" viewBox="0 0 24 24" fill="none">
                <path d="M9.5 2C7 2 5 4 5 6.5c0 .8.2 1.5.5 2.1C4 9.2 3 10.5 3 12c0 1.2.5 2.3 1.3 3-.2.4-.3.9-.3 1.5C4 18.5 5.5 20 7.5 20c.3 0 .6 0 .9-.1.6.7 1.5 1.1 2.6 1.1h2c1.1 0 2-.4 2.6-1.1.3.1.6.1.9.1 2 0 3.5-1.5 3.5-3.5 0-.6-.1-1.1-.3-1.5C21.5 14.3 22 13.2 22 12c0-1.5-1-2.8-2.5-3.4.3-.6.5-1.3.5-2.1C20 4 18 2 15.5 2c-1 0-1.9.3-2.6.9-.3-.1-.6-.1-.9-.1s-.6 0-.9.1C10.4 2.3 9.5 2 9.5 2z"
                  stroke="white" strokeOpacity=".9" strokeWidth="1.4" strokeLinejoin="round" fill="rgba(255,255,255,.1)" />
          ...
          {/* Back */}
          <div style={{ position: "absolute", width: 60, height: 60, background: face.back,   transform: "rotateY(180deg) translateZ(30px)", border: "1px solid rgba(255,255,255,.05)" }}>
            <div style={{ position: "absolute", inset: 0, animation: `cube-shade-b ${isDoc ? "8s" : "8s"} cubic-bezier(.37,0,.63,1) infinite`, animationDelay: isDoc ? "-2s" : "0s" }} className={isDoc ? "shade-d-b" : "shade-c-b"} />
          </div>
          {/* Right */}
          <div style={{ position: "absolute", width: 60, height: 60, background: face.right,  transform: "rotateY(90deg) translateZ(30px)",  border: "1px solid rgba(255,255,255,.05)" }}>
            <div style={{ position: "absolute", inset: 0 }} className={isDoc ? "shade-d-r" : "shade-c-r"} />
          </div>
          {/* Left */}
          <div style={{ position: "absolute", width: 60, height: 60, background: face.left,   transform: "rotateY(-90deg) translateZ(30px)", border: "1px solid rgba(255,255,255,.05)" }}>
            <div style={{ position: "absolute", inset: 0 }} className={isDoc ? "shade-d-l" : "shade-c-l"} />
          </div>
          {/* Top */}
          <div style={{ position: "absolute", width: 60, height: 60, background: face.top,    transform: "rotateX(90deg) translateZ(30px)",  border: "1px solid rgba(255,255,255,.05)" }}>
            <div style={{ position: "absolute", inset: 0 }} className={isDoc ? "shade-d-t" : "shade-c-t"} />
          </div>
          {/* Bottom */}
          <div style={{ position: "absolute", width: 60, height: 60, background: face.bottom, transform: "rotateX(-90deg) translateZ(30px)", border: "1px solid rgba(255,255,255,.05)" }}>
            <div style={{ position: "absolute", inset: 0 }} className={isDoc ? "shade-d-bt" : "shade-c-bt"} />
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        width: 56, height: 7, borderRadius: "50%", filter: "blur(9px)",
        background: shadowColor,
        animation: `cube-shadow 6s cubic-bezier(.45,.05,.55,.95) infinite`,
        animationDelay: isDoc ? "-1.8s" : "0s",
      }} />

      {/* Keyframes injected once via a style tag */}
      <style>{`
        @keyframes cube-glow-breathe {
          0%,100%{ opacity:.7;  transform:scale(1);    }
          50%    { opacity:1;   transform:scale(1.18); }
        }
        @keyframes cube-particle {
          0%  { transform:translateY(4px) scale(1);    opacity:0; }
          10% { opacity:1; }
          90% { opacity:1; }
          100%{ transform:translateY(-38px) scale(.15); opacity:0; }
        }
        @keyframes cube-shadow {
          0%  {opacity:.55;transform:translateX(-50%) scaleX(.92)}
          18% {opacity:.2; transform:translateX(-50%) scaleX(.4) }
          35% {opacity:.6; transform:translateX(-50%) scaleX(1)  }
          52% {opacity:.18;transform:translateX(-50%) scaleX(.38)}
          68% {opacity:.62;transform:translateX(-50%) scaleX(1.04)}
          83% {opacity:.22;transform:translateX(-50%) scaleX(.44)}
          100%{opacity:.55;transform:translateX(-50%) scaleX(.92)}
        }
        ...
        @keyframes tumble-c {
          ...
          14% {transform:rotateX(24deg)  rotateY(58deg)  rotateZ(6deg) }
          28% {transform:rotateX(-10deg) rotateY(115deg) rotateZ(-5deg)}
          42% {transform:rotateX(20deg)  rotateY(172deg) rotateZ(9deg) }
          56% {transform:rotateX(-14deg) rotateY(218deg) rotateZ(-7deg)}
          70% {transform:rotateX(22deg)  rotateY(272deg) rotateZ(4deg) }
          84% {transform:rotateX(-6deg)  rotateY(318deg) rotateZ(-3deg)}
          100%{transform:rotateX(10deg)  rotateY(360deg) rotateZ(0deg) }
        }
        @keyframes tumble-d {
          0%  {transform:rotateX(-8deg)  rotateY(0deg)   rotateZ(0deg) }
          16% {transform:rotateX(18deg)  rotateY(62deg)  rotateZ(-8deg)}
          32% {transform:rotateX(-16deg) rotateY(128deg) rotateZ(6deg) }
          46% {transform:rotateX(12deg)  rotateY(186deg) rotateZ(-5deg)}
          60% {transform:rotateX(-20deg) rotateY(244deg) rotateZ(10deg)}
          74% {transform:rotateX(14deg)  rotateY(300deg) rotateZ(-6deg)}
          88% {transform:rotateX(-5deg)  rotateY(342deg) rotateZ(3deg) }
          100%{transform:rotateX(-8deg)  rotateY(360deg) rotateZ(0deg) }
        }
        .shade-c-r { animation:sc-r 8s cubic-bezier(.37,0,.63,1) infinite; }
        .shade-c-l { animation:sc-l 8s cubic-bezier(.37,0,.63,1) infinite; }
        .shade-c-t { animation:sc-t 8s cubic-bezier(.37,0,.63,1) infinite; }
        .shade-c-b { animation:sc-b 8s cubic-bezier(.37,0,.63,1) infinite; }
        .shade-c-bt{ animation:sc-bt 8s cubic-bezier(.37,0,.63,1) infinite; }
        .shade-d-r { animation:sc-r 8s cubic-bezier(.37,0,.63,1) infinite; animation-delay:-2s; }
        .shade-d-l { animation:sc-l 8s cubic-bezier(.37,0,.63,1) infinite; animation-delay:-2s; }
        .shade-d-t { animation:sc-t 8s cubic-bezier(.37,0,.63,1) infinite; animation-delay:-2s; }
        .shade-d-b { animation:sc-b 8s cubic-bezier(.37,0,.63,1) infinite; animation-delay:-2s; }
        .shade-d-bt{ animation:sc-bt 8s cubic-bezier(.37,0,.63,1) infinite; animation-delay:-2s; }
...
function EmptyState({ onSuggest, contextSummary, chatMode, kbId }: {
  ...
  // RAG / document mode — no tool suggestions, just a clean doc-focused prompt
  if (chatMode === "rag") {
    return (
  ...
  // Unified / main chat — finance + inventory quick actions
  const fin = contextSummary?.finance;
  const inv = contextSummary?.inventory;
  ...
  const suggestions = [
    ...
    inv && inv.low_stock_count > 0
      ? {
          ...
          text: `Ada ${inv.low_stock_count} produk stok menipis. Tampilkan daftarnya.`,
          color: "#f59e0b",
        }
      : {
          icon: <Package size={14} />,
          text: "Tampilkan ringkasan stok inventori saya.",
          color: "#8b5cf6",
        },
    {
      ...
      text: fin
        ...
        : "Jelaskan fitur utama aplikasi LOQA Work.",
    ...
    <div className="flex flex-col items-center justify-center min-h-full py-16 px-6 gap-10">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-center">
          ...
          <p className="text-sm leading-relaxed max-w-xs" style={{ color: "rgb(var(--tx-muted))" }}>
            Asisten bisnis all-in-one. Tanya soal keuangan, inventori, pesanan — semua dari sini.
          </p>
        </div>
        ...
        {(fin || inv) && (
          <div className="flex items-center gap-2 flex-wrap justify-center">
            ...
            {inv && inv.low_stock_count > 0 && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium"
                style={{ background: "rgb(245 158 11 / 0.1)", border: "1px solid rgb(245 158 11 / 0.2)", color: "#f59e0b" }}>
                <AlertTriangle size={10} />
                {inv.low_stock_count} stok menipis
              </div>
            )}
            {inv && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium"
                style={{ background: "rgb(139 92 246 / 0.1)", border: "1px solid rgb(139 92 246 / 0.2)", color: "#a78bfa" }}>
                <Package size={10} />
                Inventori tersambung
              </div>
            )}
          </div>
        )}
