...
const DEFAULT_THEME: ThemeConfig = { mode: "dark", accent: "#7c3aed" };
...
function applyAccent(hex: string) {
  ...
  // Generate a perceptually balanced scale:
  // 600 = the chosen color (base)
  // 700 = darker (-12% L, +3% S)
  // 500 = lighter (+8% L, -3% S)
  // 400 = lighter (+18% L, -8% S)
  // 300 = lighter (+28% L, -14% S)
  // 50  = very light tint
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("business_app_theme");
      if (stored) {
  ...
  const setTheme = useCallback((t: ThemeConfig) => {
    setThemeState(t);
    applyTheme(t);
    localStorage.setItem("business_app_theme", JSON.stringify(t));
  }, []);
