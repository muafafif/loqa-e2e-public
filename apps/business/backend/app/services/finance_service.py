...
def _run_migrations(conn: DbConn):
    migrations = [
        ...
        # pl_type: NULL=unclassified, 'cogs'=HPP, 'gross_income'=Pendapatan Kotor/operasional
        "ALTER TABLE categories ADD COLUMN pl_type TEXT",
        # transaction_fees: per-transaction fee/discount line items
...
def list_categories(type_filter: Optional[str] = None) -> list[dict]:
    ...
    if type_filter:
        rows = conn.execute(
            "SELECT id, name, type, color, icon, pl_type, created_at FROM categories WHERE type=? ORDER BY name ASC",
            (type_filter,),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT id, name, type, color, icon, pl_type, created_at FROM categories ORDER BY type ASC, name ASC"
        ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_category(category_id: int) -> Optional[dict]:
    conn = _get_conn()
    row = conn.execute(
        "SELECT id, name, type, color, icon, pl_type, created_at FROM categories WHERE id=?",
        (category_id,),
    ).fetchone()
    conn.close()
    return dict(row) if row else None
...
def create_category(data: dict) -> dict:
    ...
    conn = _get_conn()
    cur = conn.execute(
        "INSERT INTO categories(name, type, color, icon, pl_type, created_at) VALUES (?,?,?,?,?,?)",
        (data["name"], data["type"], data.get("color", "#6b7280"),
         data.get("icon", "tag"), data.get("pl_type"), now),
    )
    conn.commit()
...
def list_transactions(
    ...
    for r in rows:
        d = dict(r)
        d["fees"] = _get_fees(d["id"], conn)
        d["fee_total"] = sum(f["amount"] for f in d["fees"])
        d["grand_total"] = d["amount"] + d["fee_total"]
        result.append(d)
    conn.close()
    return result
...
def _get_fees(tx_id: int, conn: DbConn) -> list[dict]:
    rows = conn.execute(
        "SELECT id, label, fee_type, amount FROM transaction_fees WHERE transaction_id=? ORDER BY id ASC",
        (tx_id,),
    ).fetchall()
    return [dict(r) for r in rows]


def _upsert_fees(tx_id: int, fees: list[dict], conn: DbConn):
    """Replace all fees for a transaction atomically."""
    conn.execute("DELETE FROM transaction_fees WHERE transaction_id=?", (tx_id,))
    now = _now()
    for f in fees:
        fee_type = f.get("fee_type", "other")
        # discount: user inputs positive, stored as negative
        amount = float(f["amount"])
        if fee_type == "discount":
            amount = -abs(amount)
        conn.execute(
            "INSERT INTO transaction_fees(transaction_id, label, fee_type, amount, created_at) VALUES (?,?,?,?,?)",
            (tx_id, f["label"], fee_type, amount, now),
        )
...
def get_transaction(tx_id: int) -> Optional[dict]:
    ...
    if not row:
        ...
        return None
    result = dict(row)
    result["fees"] = _get_fees(tx_id, conn)
    result["fee_total"] = sum(f["amount"] for f in result["fees"])
    result["grand_total"] = result["amount"] + result["fee_total"]
    conn.close()
    return result
...
def create_transaction(data: dict) -> dict:
    ...
    with conn:
        ...
        if data.get("fees"):
            _upsert_fees(tx_id, data["fees"], conn)
...
def update_transaction(tx_id: int, data: dict) -> Optional[dict]:
    ...
    with conn:
        ...
        # Replace fees if provided (None = leave unchanged, [] = clear all)
        if "fees" in data:
            _upsert_fees(tx_id, data["fees"] or [], conn)
...
def get_pl_report(
    ...
    """
    Monthly P&L: gross_income, cogs, gross_profit, opex, net_profit, net_margin.

    Categories are classified by pl_type:
      - pl_type='gross_income' + type='income'  → Pendapatan Kotor (operating revenue)
      - pl_type='cogs'         + type='expense' → HPP/COGS
      - pl_type=NULL           + type='income'  → Pendapatan Lain-lain (other income)
      - pl_type=NULL           + type='expense' → Beban Operasional
    """
    extra = []
    params: list = [f"-{months} months"]
    if pocket_id is not None:
        extra.append("AND t.pocket_id=?"); params.append(pocket_id)
    if account_id is not None:
        extra.append("AND t.account_id=?"); params.append(account_id)
    extra_sql = " ".join(extra)
    ...
    rows = conn.execute(
        f"""
        SELECT
            {conn.month_format('t.date')} AS month,
            COALESCE(SUM(CASE
                WHEN t.type='income'  AND c.pl_type='gross_income'
                THEN t.amount ELSE 0 END), 0) AS gross_income,
            COALESCE(SUM(CASE
                WHEN t.type='expense' AND c.pl_type='cogs'
                THEN t.amount ELSE 0 END), 0) AS cogs,
            COALESCE(SUM(CASE
                WHEN t.type='expense' AND (c.pl_type IS NULL OR c.pl_type != 'cogs')
                THEN t.amount ELSE 0 END), 0) AS opex,
            COALESCE(SUM(CASE
                WHEN t.type='income'  AND (c.pl_type IS NULL OR c.pl_type != 'gross_income')
                THEN t.amount ELSE 0 END), 0) AS other_income
        FROM transactions t
        LEFT JOIN categories c ON t.category_id = c.id
        WHERE t.date >= date('now', ?) AND t.type IN ('income','expense')
        {extra_sql}
        GROUP BY month
        ORDER BY month ASC
        """,
    ...
    for r in rows:
        ...
        net_profit   = gross_profit - opex + other_income
        net_margin   = round(net_profit / gross_income * 100, 2) if gross_income > 0 else 0.0
        gross_margin = round(gross_profit / gross_income * 100, 2) if gross_income > 0 else 0.0
        result.append({
            "month":        r["month"],
